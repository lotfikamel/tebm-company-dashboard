...
import { fetchDriverInfos } from "../../Queries/DriversQueries";

import { formatFullName, modalAction, toast } from "../../ComponentsUtilities/ComponentsUtilities";
			...
			<div className="modal">
				...
				<div className="modal-box max-w-3xl space-y-4">
					...
					{ results.length === 0 ? (
						...
						<p className="text-gray-500 text-xl text-center">Aucun chauffeur trouvé.</p>
					) : (
						...
						<div>
							...
							{ results.map(driver => (
								...
								<Link to={`/drivers/profile/${driver._id}`} key={driver._id}>

									<div className="flex cursor-pointer">

										<img src={driver.photo.src} alt="" className="w-16 rounded-full aspect-ratio-square"/>

										<div className="ml-2">

											<div className="font-medium">{ formatFullName(driver.firstName, driver.lastName) }</div>

											<div className="text-stone-400">depuis { moment(driver.creationDate).format('DD MMMM YYYY HH:mm') }</div>
...
const Search = () => {
	...
	const search = async () => {
		...
		if (input.trim().length === 0) {
			...
			toast({ message : 'Veuillez saisir un code chauffeur' })
		...
		try {
			...
			const data = await fetchDriverInfos({
